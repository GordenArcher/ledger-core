package main

import (
	"fmt"
	"log"

	"github.com/GordenArcher/ledger-core/config"
	"github.com/GordenArcher/ledger-core/internal/account"
	"github.com/GordenArcher/ledger-core/internal/transfer"
	"github.com/GordenArcher/ledger-core/pkg/database"
	"github.com/gin-gonic/gin"
)

func main() {
	// Load all configuration from environment / .env file
	cfg := config.Load()

	// Connect to PostgreSQL, will panic if connection fails
	db := database.Connect(cfg.DatabaseURL)

	// Run GORM auto-migrations — creates or updates tables to match models.
	// Safe to run on every startup; GORM only adds missing columns/indexes,
	// it never drops existing ones.
	if err := db.AutoMigrate(
		&account.Account{},
		&transfer.Transfer{},
	); err != nil {
		log.Fatalf("Auto-migration failed: %v", err)
	}
	log.Println("Database migration complete")

	// Initialize Gin with Logger and Recovery middleware
	router := gin.Default()

	// Health check
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{"status": "ok", "service": "ledger-core"})
	})

	// API v1 route group
	v1 := router.Group("/api/v1")

	// Wire account dependencies: Handler → Service → Repository → DB
	accountRepo := account.NewRepository(db)
	accountService := account.NewService(accountRepo)
	account.RegisterRoutes(v1, accountService)

	// Wire transfer dependencies.
	// The transfer service receives both the transfer repo and the account repo
	// because a transfer must atomically update two account balances.
	transferRepo := transfer.NewRepository(db)
	transferService := transfer.NewService(db, transferRepo, accountRepo)
	transfer.RegisterRoutes(v1, transferService)

	// Start server
	addr := fmt.Sprintf(":%s", cfg.ServerPort)
	log.Printf("ledger-core starting on %s", addr)

	if err := router.Run(addr); err != nil {
		log.Fatalf("Failed to start server: %v", err)
	}
}
